...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto mt-10 p-8 bg-white max-w-lg shadow-md rounded-lg">
        <h1 class="text-3xl font-bold mb-6">Sửa Danh mục: <?= htmlspecialchars($category->name) ?></h1>
        <form action="/shoe-shop/public/admin/categories/edit/<?= $category->id ?>" method="POST" enctype="multipart/form-data">
            <div class="mb-4">
                <label for="name" class="block text-gray-700 font-bold mb-2">Tên Danh mục</label>
                <input type="text" name="name" id="name" value="<?= htmlspecialchars($category->name) ?>" class="shadow border rounded w-full py-2 px-3" required>
            </div>
            <div class="mb-4">
                ...
                <?php if ($category->image_url): ?>
                    <img src="<?= htmlspecialchars($category->image_url) ?>" alt="Ảnh hiện tại" class="w-32 h-32 object-cover rounded mt-2">
                <?php endif; ?>
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 font-bold mb-2">Trạng thái</label>
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" value="1" class="mr-2 h-5 w-5" <?= $category->is_active ? 'checked' : '' ?>>
